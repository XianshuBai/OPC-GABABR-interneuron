function SpAcAnUpDown_sIPSCinOPC
...
function Para=detectionParameters
    leading=0.2; % second
    ScaleN=[0.05,0.05,0.5];% ScaleN=[0.0002,0.001,0.02];  % 1 ms, 0.5ms, 2ms.
    scale=1e12;
    smoothWindow=0.1; % second
    threshold=3.5;
    lengthAvgAll=10;
...
function setPara(varargin)
    ...
    upordown = inputdlg({'Leading time (s):','ScaleN (s)','scale',...
        'smooth window (s)','threshold','length for Average (s):'},...
        'Parameters',...
        1,...
...
function [results,Ismooth,avg,avgsmooth]=SpAcAnAnalysis(tStep,I,variableName,currNo,upordown,Para)
    ...
    for k=1:bwMax
        currbw=bw==k;
        xmin=min(x(currbw));
        currleadingPntMin = xmin - leadingPnt;
        if currleadingPntMin <1
            currleadingPntMin = 1;
        end
        
        % Find next peak.
        while ((xmin-1)>currleadingPntMin) && (~(bw(xmin-1)))
            xmin=xmin-1;
        end
        tStart=xmin;
        ...
        % Find next peak.
        tStop = max(x(currbw)) + leadingPnt * 2;
        if tStop>tlen
            tStop = tlen;
        end
        while ((xmax+1)<=tStop) && (~(bw(xmax+1)))
            xmax=xmax+1;
        end
        tStop=xmax;
